...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        # (x - y)^2 -> x^2 + y^2 - 2xy
        distances = np.sqrt((X**2).sum(axis=1, keepdims=True) + (self.X_train**2).sum(axis=1) - 2 * X.dot(self.X_train.T))
    ...
    def predict_labels(self, dists, k=1):
        ...
        number_test = dists.shape[0]
        y_predictions = np.zeros(number_test)
        ...
        for i in range(number_test):
            ...
            y_predictions[i] = Counter(closest_y).most_common(1)[0][0]
