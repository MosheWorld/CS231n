...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10, dropout=1, normalization=None, reg=0.0, weight_scale=1e-2, dtype=np.float32, seed=None):
        """
        ...
        dimensions = [input_dim] + hidden_dims + [num_classes]  # All the dimensions of network.
        ...
        if self.normalization == "layernorm":
            self.bn_params = [{"layernorm": True} for i in range(self.num_layers - 1)]
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):  # Last layer is just affine_forward without ReLU.

            # Forward pass.
            layer_input, cache = affine_forward(layer_input, self.params["W" + str(i+1)], self.params["b" + str(i+1)])
            caches.append(cache)
            
            # If batch normalization is True we will pass this layer threw batchnorm_forward.
            if (self.normalization):
            ...
            # Apply ReLU non-linearity function.
            layer_input, cache = relu_forward(layer_input)
            relu_caches.append(cache)

            # If dropout is True we will pass this layer threw dropout_forward.
            if (self.use_dropout):
                layer_input, d_cache = dropout_forward(layer_input, self.dropout_param)
                dp_caches.append(d_cache)

        # At the last layer, we do not apply ReLU, Batch normalization, Dropout.
        ...
        for i in range(self.num_layers-1, 0, -1):
            ...
            # ReLU backward.
            dloss = relu_backward(dloss, relu_caches[i-1])

            # Batch normalization backward.
            if (self.normalization):
                dloss, grads["gamma" + str(i)], grads["beta" + str(i)] = batchnorm_backward_alt(dloss, bn_caches[i-1])
